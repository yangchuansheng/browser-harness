#!/usr/bin/env python3
"""Run a live end-to-end smoke test for `bhrun wait-for-load-event`.

Required:
  BROWSER_USE_API_KEY

Optional:
  BU_NAME                   defaults to "bhrun-event-smoke"
...
os.environ.setdefault("BU_NAME", "bhrun-event-smoke")
...
def main():
    ...
    try:
        ...
        result["new_tab_target"] = new_tab("https://example.com/?via=bhrun-event-smoke")
        ...
        target_url = f"https://example.com/?via=bhrun-event-smoke&token={token}"
        ...
        if not wait_result.get("matched"):
            raise RuntimeError("wait-for-event returned matched=false")
        ...
        if result["after_wait_page"].get("url") != target_url:
            raise RuntimeError("page URL did not match the navigation triggered for wait-for-event")
