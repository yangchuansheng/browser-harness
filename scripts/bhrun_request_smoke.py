#!/usr/bin/env python3
"""Run a live end-to-end smoke test for `bhrun wait-for-request`.

Optional:
  BU_NAME                   defaults to "bhrun-request-smoke"
  BU_BROWSER_MODE           defaults to "local"; "remote" remains best-effort only
...
os.environ.setdefault("BU_NAME", "bhrun-request-smoke")
...
def main():
    ...
    try:
        ...
        base_url = "https://example.com/?via=bhrun-request-smoke"
        result["initial_page"] = page_info(daemon_name=name)
        result["new_tab_target"] = new_tab(base_url, daemon_name=name)
        result["loaded"] = wait_for_load(daemon_name=name)
        result["after_nav"] = page_info(daemon_name=name)
        ...
        target_url = f"https://example.com/?via=bhrun-request-smoke&token={token}"
        ...
        time.sleep(0.5)
        result["fetch_result"] = js(
            f"fetch({json.dumps(target_url)}, {{cache: 'no-store'}}).then(() => 'ok').catch(err => String(err))",
            daemon_name=name,
            timeout_seconds=20,
        )
        ...
        if not wait_result.get("matched"):
            raise RuntimeError("wait-for-request returned matched=false")
        if event.get("method") != "Network.requestWillBeSent":
            raise RuntimeError(f"unexpected event method: {event.get('method')!r}")
        if event.get("session_id") != result["session_id"]:
            raise RuntimeError("request event session_id did not match the active session")
        if request.get("url") != target_url:
            raise RuntimeError("request event URL did not match the triggered fetch URL")
        if request.get("method") != "GET":
            raise RuntimeError(f"unexpected request method: {request.get('method')!r}")
        result["after_wait_page"] = page_info(daemon_name=name)
        if result["after_wait_page"].get("url") != base_url:
            raise RuntimeError("page URL changed during request-side wait smoke")
