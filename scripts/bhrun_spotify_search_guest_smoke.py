#!/usr/bin/env python3
"""Run a smoke for the Rust/Wasm Spotify search guest.

Required in remote mode:
  BROWSER_USE_API_KEY

Optional:
  BU_NAME                   defaults to "bhrun-spotify-search-guest-smoke"
...
os.environ.setdefault("BU_NAME", "bhrun-spotify-search-guest-smoke")

from admin import _browser_use, ensure_daemon, restart_daemon, start_remote_daemon  # noqa: E402

TARGET_URL_PREFIX = "https://open.spotify.com/search"
DIAGNOSTIC_SCRIPT = r"""
JSON.stringify({
...
def build_guest_module(guest_manifest):
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(
            "failed to build the Rust Spotify search guest; ensure the stable wasm target is installed "
...
def main():
    ...
    guest_manifest = REPO / "rust" / "guests" / "rust-spotify-search" / "Cargo.toml"
    default_guest_path = (
        REPO
        / "rust"
        / "guests"
        ...
        / "wasm32-unknown-unknown"
        / "release"
        / "rust_spotify_search_guest.wasm"
    )
    ...
    result = {
        ...
        "skill": "domain-skills/spotify/scraping.md",
    ...
    try:
        ...
        raw_payload = calls[5].get("response")

        if int(wait_response.get("elapsed_ms", 0)) < 3000:
            raise RuntimeError(f"guest search wait did not sleep long enough: {wait_response!r}")
        if not str(page_response.get("url", "")).startswith(TARGET_URL_PREFIX):
            raise RuntimeError("guest page_info did not remain on the Spotify search page")

        payload = json.loads(raw_payload)
        tracks = payload.get("trackResults") or []
        result["search_payload"] = {
            "url": payload.get("url"),
            "track_count": len(tracks),
            "track_sample": tracks[:5],
        }
        if len(tracks) < 4:
            (
            ...
            raise RuntimeError(f"guest extracted too few Spotify track rows: {len(tracks)}")
        if "never" not in str(payload.get("url", "")).lower():
            raise RuntimeError("guest did not remain on the expected Spotify query route")
        first = tracks[0]
        if not str(first.get("href", "")).startswith("https://open.spotify.com/track/"):
            raise RuntimeError("guest extracted a malformed Spotify track URL")
        if not str(first.get("text", "")).strip():
            raise RuntimeError("guest extracted an empty Spotify track label")
        ...
        if not str(result["page_after_guest"].get("url", "")).startswith(TARGET_URL_PREFIX):
            raise RuntimeError("runner page-info after guest did not remain on the Spotify search page")
    finally:
        restart_daemon(name)
        time.sleep(1)
