#!/usr/bin/env python3
"""Run a smoke for the Rust/Wasm Letterboxd popular guest.

Required in remote mode:
  BROWSER_USE_API_KEY

Optional:
  BU_NAME                   defaults to "bhrun-letterboxd-popular-guest-smoke"
...
os.environ.setdefault("BU_NAME", "bhrun-letterboxd-popular-guest-smoke")
...
TARGET_URL_PREFIX = "https://letterboxd.com/films/popular/"
DIAGNOSTIC_SCRIPT = r"""
JSON.stringify({
...
def build_guest_module(guest_manifest):
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(
            "failed to build the Rust Letterboxd popular guest; ensure the stable wasm target is installed "
...
def main():
    ...
    guest_manifest = REPO / "rust" / "guests" / "rust-letterboxd-popular" / "Cargo.toml"
    default_guest_path = (
        REPO
        / "rust"
        / "guests"
        / "rust-letterboxd-popular"
        / "target"
        / "wasm32-unknown-unknown"
        / "release"
        / "rust_letterboxd_popular_guest.wasm"
    )
    ...
    result = {
        ...
        "skill": "domain-skills/letterboxd/scraping.md",
    ...
    try:
        ...
        raw_films = calls[-1].get("response")
        ...
        if not str(page_response.get("url", "")).startswith(TARGET_URL_PREFIX):
            raise RuntimeError("guest page_info did not remain on the Letterboxd popular URL")

        films = json.loads(raw_films)
        result["popular_count"] = len(films)
        result["popular_sample"] = films[:3]
        result["popular_with_film_id_count"] = sum(
            1 for film in films if str(film.get("film_id") or "").strip()
        )
        if len(films) < 20:
            (
            ...
            raise RuntimeError(f"guest extracted too few Letterboxd popular rows: {len(films)}")
        first = films[0]
        if not str(first.get("name", "")).strip():
            raise RuntimeError("guest extracted an empty Letterboxd name")
        if not str(first.get("slug", "")).strip():
            raise RuntimeError("guest extracted an empty Letterboxd slug")
        if not str(first.get("url", "")).startswith("https://letterboxd.com/film/"):
            raise RuntimeError("guest extracted a malformed Letterboxd film URL")
        film_id = str(first.get("film_id") or "").strip()
        if film_id and not film_id.isdigit():
            raise RuntimeError("guest extracted a malformed Letterboxd film_id")
        ...
        if not str(result["page_after_guest"].get("url", "")).startswith(TARGET_URL_PREFIX):
            raise RuntimeError("runner page-info after guest did not remain on the Letterboxd popular URL")
    finally:
        restart_daemon(name)
        time.sleep(1)
