...
> **Rust fork note:** The video pipeline (recording engine, composition, and
> export) is not yet ported to Rust. The `browser-harness video` subcommands
> described below rely on the upstream Python harness. The Rust fork will gain
> equivalent support in a future release.
